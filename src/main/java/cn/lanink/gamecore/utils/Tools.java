...
import cn.lanink.gamecore.entity.EntityPlayerCorpse;
...
public class Tools {
    ...
    public static String getStringRoomMode(Player player, BaseRoom room) {
        switch (room.getGameMode()) {
            case "classic":
                return GameCore.getInstance().getLanguage(player).Classic;
            case "infected":
                return GameCore.getInstance().getLanguage(player).Infected;
            default:
        ...
        for (String s : cmds) {
            ...
            if ((cmd.length > 1) && ("con".equals(cmd[1]))) {
                Server.getInstance().dispatchCommand(Server.getInstance().getConsoleSender(),
                        cmd[0].replace("@p", player.getName()));
            } else {
    ...
    public static Item getMurderItem(Player player, int tagNumber) {
        Item item;
        Language language = GameCore.getInstance().getLanguage(player);
        switch (tagNumber) {
            case 1:
    ...
    private static void setHumanSkin(Player player, EntityHuman human, Skin skin, boolean needACK, int retransmission) {
        ...
        if (needACK && !human.isClosed() && retransmission < 3) {
            Server.getInstance().getScheduler().scheduleDelayedTask(GameCore.getInstance(), new Task() {
                @Override
    ...
    public static void spawnCorpse(Location location) {
        spawnCorpse(location, GameCore.getInstance().getSteveSkin());
    }

    public static void spawnCorpse(Location location, Skin skin) {
        switch(skin.getSkinData().data.length) {
            ...
            case 65536:
                break;
            default:
                skin = GameCore.getInstance().getSteveSkin();
        }
        CompoundTag nbt = EntityPlayerCorpse.getDefaultNBT(location);
        nbt.putCompound("Skin", new CompoundTag()
                .putByteArray("Data", skin.getSkinData().data)
                .putString("ModelId", skin.getSkinId()));
        nbt.putFloat("Scale", -1.0F);
        EntityPlayerCorpse ent = new EntityPlayerCorpse(location.getChunk(), nbt);
        ent.setSkin(skin);
        ent.setPosition(new Vector3(location.getFloorX(), Tools.getFloorY(location), location.getFloorZ()));
        ent.setGliding(true);
        ent.setRotation(location.getYaw(), 0);
        ent.spawnToAll();
        ent.updateMovement();
    }
     ...
     * @return Y
     */
    public static double getFloorY(Position position) {
        if (position.getFloorY() <= 0) {
            return 1;
        }
        for (int y = 0; y < 15; y++) {
            Level level = position.getLevel();
            Block block = level.getBlock(position.getFloorX(), position.getFloorY() - y, position.getFloorZ());
            if (block.getId() != 0) {
    ...
    public static void spawnFirework(Position position) {
        ...
        ex.putByteArray("FireworkColor",new byte[]{
                (byte) DyeColor.values()[GameCore.RANDOM.nextInt(ItemFirework.FireworkExplosion.ExplosionType.values().length)].getDyeData()
        });
        ex.putByteArray("FireworkFade",new byte[0]);
        ex.putBoolean("FireworkFlicker", GameCore.RANDOM.nextBoolean());
        ex.putBoolean("FireworkTrail", GameCore.RANDOM.nextBoolean());
        ex.putByte("FireworkType",ItemFirework.FireworkExplosion.ExplosionType.values()
                [GameCore.RANDOM.nextInt(ItemFirework.FireworkExplosion.ExplosionType.values().length)].ordinal());
